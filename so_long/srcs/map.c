...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/02/24 01:43:10 by niclaw            #+#    #+#             */
...
static void	map_fill(t_data *data, char *f);
		...
		while (*f && *f != '\n')
		{
			if (*f != 'C' && *f != 'E' && *f != 'P' && *f != '1' && *f != '0')
				error_game(data, ERROR_MAP_INVALID, \
					"map has unidentified object or space");
...
void	load_tiles(t_data *data)
{
	...
	while (*str)
	{
		...
		while (*str != '\n' && *str)
		{
			data->map.tiles[gy][gx] = \
			new_tile(*str, gx * data->bsize, gy * data->bsize);
			gx++;
			str++;
		}
		str++;
